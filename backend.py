...
async def streaming_generate(item: GenerateParams):
    return EventSourceResponse(vic(item.prompt, streaming=True, **item.params), media_type="text/event-stream")
...
async def chat(history: list[list[str]]):
    ...
    prompt = PromptTemplate(input_variables=["history", "content"], template=template)
    ...
    prompt = prompt.format(history=history_text, content=content)

    # 构建生成参数
    params = dict(min_length=0, max_length=2048, temperature=0.1, top_p=0.75, top_k=40)

    return EventSourceResponse(vic(prompt, streaming=True, **params), media_type="text/event-stream")
